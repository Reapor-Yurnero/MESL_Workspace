...
def create_chain(container_name: str):
    ...
    cmd = "sudo iptables -N " + container_name+"-external"
    subprocess.run(split(cmd))
    cmd = "iptables -I DOCKER-USER -j " + container_name+"-external"
    subprocess.run(split(cmd))
...
def delete_chain(container_name: str):
    ...
    cmd = "iptables -D DOCKER-USER -j " + container_name+"-external"
    subprocess.run(split(cmd))
    cmd = "sudo iptables -X " + container_name+"-external"
    subprocess.run(split(cmd))
...
def grant_external_access(container_name: str, container_ip: str, protocol: str = '', dst_ip: str = '',
    ...
    spec = container_name + "-external" + " -i docker1 "
    if protocol is not '':
...
def revoke_all_access(container_name: str, option: int=0):
    ...
    if option == 0:
        cmd = "sudo iptables -F " + container_name + "-external"
        subprocess.run(split(cmd))
